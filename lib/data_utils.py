...
def shuffle_data(*arrays, **options):
    if isinstance(arrays[0][0], basestring):
        ...
        return shuffle(*arrays, random_state=np_rng)
...
def FlipImgs(xmb, ymb):
    ...
    for i in xrange(bs):
        ...
        if np.random.rand(1) > 0.5:
            ...
            x_img.transpose(Image.FLIP_LEFT_RIGHT)
            y_img.transpose(Image.FLIP_LEFT_RIGHT)
...
def inpainting_data(dmb, image_shape=[3,128,128], patch_shape=[3,64,64],overlap=4):
    nb = dmb.shape[0]
    ic, ih, iw = image_shape 
    pc, ph, pw = patch_shape
    ymb = np.zeros([nb,pc,ph,pw],dtype='uint32')
    xmb = dmb 
    ymb[:,:,:,:] = dmb[:,:,(ih-ph)/2:(ih+ph)/2,(iw-pw)/2:(iw+pw)/2]
    xmb[:,0,(ih-ph)/2+overlap:(ih+ph)/2-overlap,(iw-pw)/2+overlap:(iw+pw)/2-overlap]=117.0
    xmb[:,1,(ih-ph)/2+overlap:(ih+ph)/2-overlap,(iw-pw)/2+overlap:(iw+pw)/2-overlap]=104.0
    xmb[:,2,(ih-ph)/2+overlap:(ih+ph)/2-overlap,(iw-pw)/2+overlap:(iw+pw)/2-overlap]=123.0
    return xmb, ymb	
